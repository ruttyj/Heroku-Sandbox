...
//#######################################################
export function StateBuffer(_initialState = {}) {
    const initialState = _initialState;
    let mCurrentState = initialState;

    let counter = 0;
    let mSetter = null;
    let mHashSetter = null;
    let mMutator = (v) => v;
    const _flush = debounce(function() {
        flush(mSetter);
    }, 150);
    ...
    function flush() {
        let stateToSet = isDef(mMutator) ? mMutator(mCurrentState) : mCurrentState;

        if (isDef(mSetter)) {
            mSetter(stateToSet);
        }

        if(isDef(mHashSetter)) {
            mHashSetter(++counter);
        }
    }
